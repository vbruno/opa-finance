...
- Para ocorrências `materialized`, o `amount` exibido na timeline reflete o valor atual da tabela `transactions`, tanto para transações (`transactionId != null`, lendo a transação direta) quanto para transferências (`transferId != null`, lendo a perna `expense`); em casos extremos de ausência da transação/perna, há fallback para `reviewPayload.amount` e, por fim, para o valor da regra-mãe
...
- Quando a data já possui ocorrência persistida, `POST /recurrences/:id/anticipate` retorna mensagem específica por status: `materialized` e `pending_review` respondem `409`; `skipped` e `failed` respondem `422` orientando uso de `this_and_next` a partir de uma data futura
