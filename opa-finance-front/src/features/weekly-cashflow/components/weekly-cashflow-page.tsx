...
import { GROUP_ITEM_PREFIX, VIEW_ID, VIEW_STATE_VERSION } from '@/features/weekly-cashflow/model/weekly-cashflow.constants'
import {
  areArraysEqual,
  areGroupDisplayModesEqual,
  areGroupsEqual,
  ...
  isGroupDisplayMode,
  isIsoDateInRange,
  loadWeeklyCashflowViewState,
  maxUtcDate,
  normalizeSeparatorPositions,
  parseIsoDateToUtc,
  saveWeeklyCashflowViewState,
...
export function WeeklyCashflowPage({
  search,
  navigate,
}: {
  ...
  const { selectedAccountIds, setSearch, toggleAccount, updateSelectedAccounts, weekStart, year } =
    useWeeklyCashflowSearchParams({
      search,
      navigate,
      currentYear,
      persistedYear: persistedViewState?.year,
      persistedWeekStart: persistedViewState?.weekStart,
      persistedAccountIds: persistedViewState?.accountIds,
      allAccountIds,
      primaryAccountId,
    })
  ...
  const {
    ...
    initialSelectedColumnIds: persistedViewState?.selectedColumnIds ?? [],
    initialColumnOrder: persistedViewState?.columnOrder ?? [],
    initialGroups: persistedViewState?.groups ?? [],
    initialGroupDisplayModes: persistedViewState?.groupDisplayModes ?? {},
    initialSeparatorPositions: normalizeSeparatorPositions(
      persistedViewState?.separatorPositions,
      persistedViewState?.separatorPosition,
    ),
    initialSortDynamicByShare: persistedViewState?.sortDynamicByShare ?? true,
  })
  ...
  const validOrderItemIds = useMemo(
    () => [...selectedColumnsOrdered, ...groupsNormalized.map((group) => group.itemId)],
    [groupsNormalized, selectedColumnsOrdered],
  )
  ...
  }, [orderedItems.length, separatorPositions, setSeparatorPositions])
  ...
  }, [isColumnsConfigOpen, setEditingGroupId, setIsColumnsConfigOpen, setIsCreateGroupOpen])
  ...
  }, [isCreateGroupOpen, setEditingGroupId, setIsCreateGroupOpen, setNewGroupColumnIds, setNewGroupName])
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-2">
            ...
            <Select
              value={weekStart}
              onValueChange={(value) =>
                setSearch({ weekStart: value as WeeklyCashflowSearch['weekStart'] })
              }
            >
                ...
                <div className="h-[58dvh] space-y-1 overflow-y-auto rounded-md border p-2">
                  <div className="flex items-center justify-between px-1 pb-1">
                    ...
                    <Button
                      ...
                      onClick={() => addSeparatorItem(orderedItems.length)}
                      disabled={
                            ...
                            <div className="mb-1 flex items-center justify-between rounded border border-dashed border-border/80 bg-muted/20 px-2 py-1 text-xs">
                              ...
                              <div className="flex items-center gap-1">
                                ...
                                <Button
                                  ...
                                  onClick={() =>
                                    moveSeparatorItem(
                                      panelItem.topLevelIndex,
                                      'up',
                                      Math.max(1, selectedColumnsOrdered.length - 1),
                                    )
                                  }
                            ...
                            <div className="ml-2 flex items-center gap-2">
                                  ...
                                  onClick={() =>
                                    openEditGroupModal(
                                      ...
                                      group.columnIds.filter((columnId) =>
                                        columnsCatalogById.has(columnId),
                                      ),
                                    )
                                  }
                                  title="Editar grupo"
                              ...
                              {panelItem.kind !== 'group-child' ? (
                                <Button
                                  ...
                                  className="h-6 px-2"
                                  onClick={() =>
                                    moveOrderItem(panelItem.itemId, 'up', validOrderItemIds)
                                  }
                              ...
                              {panelItem.kind !== 'group-child' ? (
                                <Button
                                  ...
                                  className="h-6 px-2"
                                  onClick={() =>
                                    moveOrderItem(panelItem.itemId, 'down', validOrderItemIds)
                                  }
                ...
                <div className="relative w-full max-w-2xl rounded-lg border bg-background p-3 shadow-lg sm:p-4">
                  ...
                  <div className="mt-3 flex justify-end gap-2">
                    ...
                    <Button
                      ...
                      variant="outline"
                      size="sm"
                      onClick={() => createGroup(newGroupColumnIds, groupedColumnOwnerById)}
                      disabled={
