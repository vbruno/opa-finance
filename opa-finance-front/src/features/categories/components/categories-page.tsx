...
export function CategoriesPage({ search, navigate }: CategoriesPageProps) {
  ...
  const [subcategoryParent, setSubcategoryParent] = useState<Category | null>(
    null,
  )
  ...
  const [lastCategoryType, setLastCategoryType] = useUserPreference<
    string | null
  ...
  const {
    ...
    setTypeValue,
    clearFilters,
  } = useCategoriesSearchParams({ search, navigate })
  ...
  const searchSubcategoriesQuery = useQuery({
    ...
    enabled: debouncedNormalizedSearch.length > 0 && userCategories.length > 0,
  })
      ...
      .filter((category) => {
        ...
        const matchesSubcategory =
          ...
          subcategories.some((subcategory) =>
            normalizeSearch(subcategory.name).includes(
              debouncedNormalizedSearch,
            ),
          )
  ...
  const subcategoryMatchIds = useMemo(() => {
    ...
    return userCategories
      ...
      .map((category) => category.id)
  }, [debouncedNormalizedSearch, searchSubcategoriesQuery.data, userCategories])
  ...
  const typeLabels: Record<Category['type'], string> = CATEGORY_TYPE_LABELS
  const isMutating =
  ...
  const submitCreateCategoryForm = form.handleSubmit(async (formData) => {
    try {
      const created = await createCategoryMutation.mutateAsync(
        mapCreateCategoryPayload(formData),
      )
    ...
    async (formData) => {
      try {
        ...
        await createSubcategoryMutation.mutateAsync(
          mapCreateSubcategoryPayload(subcategoryParent.id, formData),
        )
  ...
  const submitEditCategoryForm = editForm.handleSubmit(async (formData) => {
    try {
      ...
      await updateCategoryMutation.mutateAsync(
        mapUpdateCategoryPayload(selectedCategory.id, formData),
      )
      setIsEditOpen(false)
      ...
      <div className="flex items-center justify-between gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex shrink-0 flex-wrap items-center gap-2 sm:w-auto">
          ...
          <button
            ...
            onClick={() => {
              toggleExpandAll(filteredCategories.map((category) => category.id))
            }}
          >
            <svg
              viewBox="0 0 16 16"
          ...
          <div className="flex flex-1 flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center">
            <div className="w-full sm:min-w-[220px] sm:flex-1">
              <Input
                ...
                value={searchTerm}
                onChange={(event) => setSearchValue(event.target.value, true)}
                onKeyDown={(event) => {
                  if (event.key !== 'Enter') {
                    return
                  }
                  setSearchValue(event.currentTarget.value, false)
                }}
              />
            </div>
            ...
            >
              ...
              {expandedCategoryIds.includes(category.id) && (
                <div className="mt-2 space-y-2">
                  ...
                  {!expandedQueriesById[category.id]?.isLoading &&
                    !expandedQueriesById[category.id]?.isError &&
                    (expandedQueriesById[category.id]?.data ?? []).length ===
                      0 && (
                      <div className="rounded-md border px-3 py-2 text-sm text-muted-foreground">
            ...
            <div className="mt-6 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
              ...
              {deleteError?.toLowerCase().includes('subcategorias') && (
                <Button
                  ...
                  className="w-full sm:w-auto"
                  disabled={
                    isCascadeDeleting || deleteCategoryMutation.isPending
                  }
                  onClick={async () => {
                    if (!selectedCategory) {
                      return
