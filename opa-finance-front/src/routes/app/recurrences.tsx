...
function RecurrencesPage() {
  ...
  function setSearch(
    next: Partial<{
      page: number
      limit: number
      originType: 'transaction' | 'transfer' | undefined
      status: 'active' | 'finalized' | undefined
      frequency: 'weekly' | 'biweekly' | 'monthly' | 'yearly' | undefined
  ...
  function openEditModal(recurrence: Recurrence) {
    ...
    setFormError(null)
    setConflictRecurrenceId(null)
    form.reset(getFormValuesFromRecurrence(recurrence))
    setIsFormOpen(true)
  }
  ...
  async function handleReloadAfterConflict() {
    ...
    try {
      ...
      form.reset(getFormValuesFromRecurrence(refreshedRecurrence))
      setConflictRecurrenceId(null)
  ...
  async function onSubmit(values: RecurrenceFormData) {
    ...
    try {
      ...
      const updatePayload: RecurrenceUpdatePayload = {
        ...buildScopedUpdatePayload(values, editingRecurrence),
      }
